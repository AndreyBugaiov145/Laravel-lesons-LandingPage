                        ...
                        <div class="inner_section">
                            <div class="row">
                                <div class=" col-lg-4 col-md-4 col-sm-4 col-xs-12 pull-right">
                                    {!! Html::image('/img/'.$page->images,'',array('class'=>'img-circle delay-03s animated wow zoomIn')) !!}
                                </div>
                                <div class=" col-lg-7 col-md-7 col-sm-7 col-xs-12 pull-left">
                                    ...
                                    <div class="work_bottom"><span>Want to know more..</span>
                                        <a href="{{route('page',['alias'=>$page->alias])}}" class="contact_btn">Contact
                                            Us</a>
                                    </div>
                                </div>
...
@if(isset($serveces) && is_object($serveces))
    <!--Service-->
    <section id="service">
        <div class="container">
            <h2>Services</h2>
            <div class="service_wrapper">
                @foreach($serveces as $key => $service)
                    @if($key ==0 || $key%3==0  )
                    ...
                    @if(($key+1)%3 ==0 || $key == count($serveces))
                        </div>
                    @endif
                @endforeach
            </div>
...
@if(isset($partfolios) && is_object($partfolios))
    ...
    <section id="Portfolio" class="content">
        ...
        <!--/Portfolio Filters -->
            ...
            @foreach($partfolios as $partfolio )
                <!-- Portfolio Item -->
                    <div style="position: absolute; left: 0px; top: 0px; transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1); width: 337px; opacity: 1;"
                         class="portfolio-item one-four {{$partfolio->filter}} isotope-item">
                        <div class="portfolio_img">
                            {{Html::image('img/'.$partfolio->images)}}
                        </div>
                        <div class="item_overlay">
                            <div class="item_info">
                                <h4 class="project_name">{{$partfolio->name}}</h4>
                            </div>
                        </div>
                    </div>
                    <!--/Portfolio Item -->
...
<section class="page_section team" id="team"><!--main-section team-start-->
    <div class="container">
        ...
        <div class="team_section clearfix">
            ...
            <div class="team_area">
                <div class="team_box wow fadeInDown delay-0{{$key*3}}s">
                ...
                <h3 class="wow fadeInDown delay-0{{$key*3}}s">{{$people->name}}</h3>
                <span class="wow fadeInDown delay-0{{$key*3}}s">{{$people->position}}</span>
                <p class="wow fadeInDown delay-0{{$key*3}}s">{{$people->text}}</p>
            </div>
            @endforeach
        </div>
    </div>
